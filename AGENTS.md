...
- Always explicitly ask for and receive the user's approval of the finished entry or identified batch before adding any new timeline entries. Present the researched text, dates, sources, tags, duplicate decisions, and Beer Map coverage first. An initial request to research/add entries, standing automation permission, or build/commit/push authorization does not replace this approval. Approval covers only the reviewed proposal; ask again if its editorial content changes or additional entries are included. This requirement also applies to any future automated publication workflow and does not relax the database restrictions below.
...
- Before any backend/Supabase action, explicitly ask the user here in the conversation, describe the exact operations and affected proposal or records, and wait for an explicit affirmative answer. This includes read-only backend queries, backups, SQL/API mutations, schema/configuration changes, and builds or workflows that access Supabase. A clearly described bundle may be approved together. Never infer approval from silence, an initial task request, content approval alone, or standing automation/Git permissions.
- After the user explicitly approves that concrete backend action or bundle, execute only the approved operations. Do not ask again for the same approved action, broaden its scope, apply revised SQL without renewed approval, or automatically retry a mutation whose outcome is uncertain. This is action-specific authorization, never permission to act independently on the backend.
- The `beer-entry` and `tag-cleanup` skills may prepare safe, copy-paste-ready SQL proposals. These remain unexecuted proposals until approved. The user may execute them manually or explicitly authorize assistant execution through the conversational approval gate above; generating SQL never authorizes executing it. Preserve review-only skill boundaries unless the user explicitly requests implementation.
